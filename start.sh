...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the geez username if it exists
    if id geez &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER geez
    fi
    ...
    if [[ "$NB_USER" != "geez" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/geez "/home/$NB_USER"
        fi
        # if workdir is in /home/geez, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/geez/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u geez)" && "$NB_GID" == "$(id -g geez)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^geez:/nayvoj:/" > /tmp/passwd
                echo "geez:x:$(id -u):$(id -g):,,,:/home/geez:/bin/bash" >> /tmp/passwd
